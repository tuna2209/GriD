...
class _BarColorPickerState extends State<BarColorPicker> {
  ...
  void handleTouch(Offset globalPosition, BuildContext context) {
    ...
    double step = 100 / colors.length;
    switch (widget.pickMode) {
      case PickMode.Color:
        double p = percent * 100;
        double saturation =
            p >= step * colors.length ? ((p % step) / step) : 1.0;
        double value = p < step ? (p / step) : 1.0;
//        print('$p - $saturation - $value');
        Color color =
            HSVColor.fromAHSV(1.0, percent * 360, saturation, value).toColor();
...
class MinBoxer {
  ...
  void clamp(Matrix4 m) {
    ...
    if (dst.width < bounds.width || dst.height < bounds.height) {
      ...
      var scale = src.width / fs.destination.width;
